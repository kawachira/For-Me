...
with col_form:
    with st.form(key='search_form'):
        ...
        # <--- ส่วนที่เพิ่มเข้ามา: Checkbox Real-time --->
        st.markdown("---")
...
# --- 5. Get Data ---
@st.cache_data(ttl=10, show_spinner=False) # ปรับ TTL เป็น 10 วิ เพื่อรองรับ Loop
...
if submit_btn:
    ...
    # <--- สร้าง Placeholder เพื่อรอรับผลลัพธ์ที่จะวนลูป --->
    ...
    while True:
        ...
        # <--- จุดควบคุม Loop --->
        # ถ้าไม่ได้ติ๊ก Real-time mode ให้หยุด Loop ทันที (ทำงานรอบเดียวเหมือนเดิม)
        if not realtime_mode:
            break
        
        # ถ้าติ๊ก Real-time ให้รอ 10 วินาที แล้ววนกลับไปดึงข้อมูลใหม่
        time.sleep(10)
